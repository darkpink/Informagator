...
{
    [Serializable]
    public class DatabaseThreadConfiguration : IThreadConfiguration
    {
        public DatabaseThreadConfiguration()
        {
            StageConfigurations = new List<IStageConfiguration>();
        }

        public DatabaseThreadConfiguration(IThreadConfiguration configuration)
        {
        ...
        {
            bool result = true;
            DatabaseThreadConfiguration castedConfig = config as DatabaseThreadConfiguration;
        ...
        protected bool HasCustomConfigurationDifferences(IThreadConfiguration config)
        {
            ...
            if (config is DatabaseThreadConfiguration)
            {
                result = false;
                DatabaseThreadConfiguration castedConfig = (DatabaseThreadConfiguration)config;
